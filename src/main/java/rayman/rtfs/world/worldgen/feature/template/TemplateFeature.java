package rayman.rtfs.world.worldgen.feature.template;
...
import rayman.rtfs.server.RTFMinecraftServer;
import rayman.rtfs.world.worldgen.feature.template.TemplateFeature.Config;
...
import rayman.rtfs.world.worldgen.feature.template.paste.Paste;
import rayman.rtfs.world.worldgen.feature.template.paste.PasteConfig;
import rayman.rtfs.world.worldgen.feature.template.paste.PasteType;
...
import rayman.rtfs.world.worldgen.feature.template.template.FeatureTemplate;
    ...
    public static <T extends TemplateContext> boolean paste(WorldGenLevel world, RandomSource rand, BlockPos pos, Mirror mirror, Rotation rotation, Config<T> config, PasteType pasteType, boolean modified) {
        ...
        if(world.getServer() instanceof RTFMinecraftServer rtfMinecraftServer) {
	        ...
	        if (paste.apply(world, buffer, pos, mirror, rotation, placement, config.paste())) {
	            ResourceLocation biome = world.getBiome(pos).unwrapKey().map(ResourceKey::location).orElse(null);
		...
		RTFCommon.LOGGER.warn("TemplateFeature: server does not implement RTFMinecraftServer, skipping template placement at {}", pos);
		return false;
        }
    }
