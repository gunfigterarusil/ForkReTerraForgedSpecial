package rayman.rtfs.world.worldgen.feature;
...
import rayman.rtfs.tags.RTFBlockTags;
...
import rayman.rtfs.world.worldgen.feature.ErodeFeature.Config;
...
public class ErodeFeature extends Feature<Config> {
	...
	public boolean place(FeaturePlaceContext<Config> placeContext) {
		...
		if((Object) randomState instanceof RTFRandomState rtfRandomState && (generatorContext = rtfRandomState.generatorContext()) != null) {
			if(generatorContext.cache == null) return false;
			...
			if(providerTile == null) return false;
			Tile.Chunk tileChunk = providerTile.getChunkReader(chunkX, chunkZ);
			rayman.rtfs.world.worldgen.cell.heightmap.Heightmap heightmap = generatorContext.generator.getHeightmap();
    ...
    public record Config(int rockVar, int rockMin, int dirtVar, int dirtMin, float rockSteepness, float dirtSteepness, float screeSteepness, float heightModifier, float slopeModifier, float sedimentModifier, float sedimentNoise, float screeValue) implements FeatureConfiguration {
		public static Config makeDefault() {
			return new Config(30, 140, 40, 95, 0.65F, 0.475F, 0.4F, 6F / 255F, 3F / 255F, 256, 3F / 255F, 0.55F);
		}
