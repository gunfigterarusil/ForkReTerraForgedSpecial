...
import rayman.rtfs.world.worldgen.feature.SwampSurfaceFeature.Config;
...
public class SwampSurfaceFeature extends Feature<Config> {
	...
	// Replacement for deprecated Biome.BIOME_INFO_NOISE (same scale, consistent seed)
	private static final Noise SWAMP_INFO_NOISE = Noises.simplex(2345, 4, 1);
	...
	public boolean place(FeaturePlaceContext<Config> ctx) {
		...
		for(int x = 0; x < 16; x++) {
			for(int z = 0; z < 16; z++) {
				...
				double noise = SWAMP_INFO_NOISE.compute(worldX * 0.25F, worldZ * 0.25F, 0);
