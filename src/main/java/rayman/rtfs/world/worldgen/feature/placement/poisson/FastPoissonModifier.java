package rayman.rtfs.world.worldgen.feature.placement.poisson;
	...
	private DensityNoise getDensityNoise(int seed, ChunkPos chunkPos, RandomState randomState) {
		...
		if (this.biomeFade > BiomeVariance.MIN_FADE) {
			if((Object) randomState instanceof RTFRandomState rtfRandomState) {
				GeneratorContext ctx = rtfRandomState.generatorContext();
				if(ctx != null && ctx.cache != null) {
					Tile providerTile = ctx.cache.provideAtChunk(chunkPos.x, chunkPos.z);
					if(providerTile != null) {
						Tile.Chunk reader = providerTile.getChunkReader(chunkPos.x, chunkPos.z);
						if (reader != null) {
